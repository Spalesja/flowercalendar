...
- **MEMORY.md** - память по проекту, после каждого этапа разработки нужно обновлять этот файл
...
- Компонент: `src/components/logo.tsx` — SVG-цветок (белые лепестки, фиолетовая серединка) + надпись "Цветочный календарь" (Mulish, font-bold, цвет accent-hover)
- На экранах < 375px надпись скрывается, остаётся только цветок
- На внутренней странице — переход на главную (`<Link href="/">`)
- На главной — сброс страницы (`window.location.reload()`)
