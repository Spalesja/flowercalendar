...
### Этап 13. Блок "Сейчас цветет" — живые данные
- Новый клиентский компонент: src/components/currently-blooming.tsx
- Fetch живых данных из `/api/currently-blooming?city=&date=` при монтировании и при смене города
- По умолчанию — Минск. Город обновляется только по нажатию кнопки "Найти цветы" в режиме "Города" (НЕ сразу при выборе в autocomplete)
- Новый стейт `bloomingCitySlug` в HeroSearch (default "minsk"), устанавливается в `handleSearch` при `mode === "cities"`
- Валидация дат НЕ блокирует обновление блока — при нажатии "Найти цветы" без дат блок обновится, а поиск результатов просто не выполнится (без сообщения об ошибке)
- Показывает ВСЕ цветущие растения с пагинацией по 4 карточки (CARDS_PER_PAGE = 4)
- Стрелочки навигации: w-8 h-8 rounded-full, появляются только если totalPages > 1, disabled с opacity-30
- Кнопка "Все цветы" внизу блока (hover: bg-[#ffb1ed]/55)
- Пустое состояние: "Сейчас ничего не цветёт в этом городе"
- Loading-скелетон: 4 пульсирующих (animate-pulse) плейсхолдера
- Предложный падеж городов через маппинг CITY_PREPOSITIONAL: Минске, Гродно, Бресте, Витебске, Гомеле, Могилёве
- Карточки кликабельны — ссылка `/plants/{slug}` (страницы растений будут на этапе 14)
- Дата окончания форматируется: `format(parseISO(date), "d MMMM", { locale: ru })`
- page.tsx упрощён до `<HeroSearch />` — моковые данные и секция "Сейчас цветет" убраны, всё рендерится внутри HeroSearch
- PlantBloomCard без изменений — размеры, стили, структура сохранены

**Правки по этапу 13 (25 апреля 2026):**

**Данные:**
- Форзиция, Нарцисс, Тюльпан — end_date продлён на +10 дней по всем 6 городам
- Добавлена Алыча (id 40, slug "alycha", Prunus cerasifera) — 6 записей цветения (id 235-240), сроки сдвинуты на -10 дней от официальных, периоды продлены на +10 дней (итого ~20 дней цветения). Итого 40 растений, 240 записей

**Карта цветения:**
- Подписи городов на карте: шрифт остался `system-ui, sans-serif`, но добавлены `stroke="#ffffff"` + `stroke-width="2"` + `paint-order="stroke"` — белая обводка букв рисуется под заливкой, убраны артефакты-засечки
- Добавлен родительный падеж "Алычи" в маппинг PLANT_GENITIVE (теперь 40 растений)

**Фото растений в карточках "Сейчас цветёт":**
- PlantBloomCard стал клиентским ("use client"), добавлен `onError` обработчик: если фото не найдено (404), показывается fallback-заглушка с 🌸
- CurrentlyBlooming передаёт `imageUrl={/images/flowers-images/${slug}.jpg}` в каждую карточку
- Фото берутся из `public/images/flowers-images/{slug}.jpg` — при добавлении нового фото оно автоматически подхватывается по slug, без изменений в коде
- Поля `hero_image` в JSON пока оставлены пустыми — будут заполнены разом после полного набора фото

**Правки по этапу 13 (6 мая 2026):**

**Данные (сдвиги сроков цветения):**
- Вишня: end_date +4 дня в Минске, Витебске, Могилёве
- Слива: end_date +4 дня в Минске, Витебске, Могилёве
- Сирень: start/end +5 дней в Бресте и Гомеле
- Пион: start/end +10 дней по всем 6 городам
- Яблоня: start/end +10 дней по всем 6 городам
- Алыча: start/end +7 дней по всем 6 городам

**Кастомизация позиции фото в карточке:**
- PlantBloomCard получил опциональный пропс `imageObjectPosition` (применяется через `style={{ objectPosition }}`)
- В CurrentlyBlooming заведён маппинг `PLANT_IMAGE_POSITION: Record<string, string>` (slug → object-position) для индивидуальных подстроек композиции фото
- Сейчас маппинг пуст — пробовали для дуба `50% calc(50% + Npx)`, но проблема была в самом фото (пустое пространство сверху), сдвиг не помог. Маппинг оставлен как готовая инфраструктура для будущих случаев

**Фото растений на момент 6 мая 2026:** добавлено 14 (pervotsvety, verba-iva, olkha, nartsiss, siren, klen, tyulpan, oduvanchik, alycha, dub, geran, sliva, vishnya, yablonya), остальные пользователь добавляет постепенно

### Киллер-фича: карта Беларуси с покраской регионов (завершено)
- Компонент: src/components/belarus-bloom-map.tsx ("use client")
- SVG-карта: public/images/belarus-map.svg (исходник docs/map.svg)
- Показывается в режиме **Цветы** после результатов поиска, для ВСЕХ растений
- 6 регионов покрашены в 6 цветов по дате начала цветения (от раннего к позднему):
  1. #FFCE88 (песочно-жёлтый) — самый ранний
  2. #FFA088 (бледно-оранжевый)
  3. #FF87B7 (розовый)
  4. #FF9FFF (бледно-сиреневый)
  5. #E787FF (сиреневый)
  6. #B787FF (фиолетовый) — самый поздний
- Маппинг город→регион SVG: gomel→BYHO, brest→BYBR, vitebsk→BYVI, grodno→BYHR, mogilev→BYMA, minsk→BYMI+BYHM
- Данные берутся из CitySearchResult[], сортировка по floweringStart
- SVG загружается через fetch, красится через DOMParser, кэшируется в useRef
- Подписи городов на карте (SVG text elements)
- Легенда справа: цветной квадрат + город + даты цветения
- Заголовок: "Карта цветения {растение в родительном падеже} по регионам Беларуси"
- Маппинг PLANT_GENITIVE для всех 40 растений (родительный падеж), включая Алычу
- Размер карты: max-w-[780px]

## Следующий этап: 14 — Страницы растений (/plants/[slug])
