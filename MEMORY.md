...
## Следующий этап: киллер-фича — карта Беларуси с покраской регионов

Пользователь предоставил SVG-карту Беларуси (by.svg) с регионами (path id: BYHO=Gomel, BYBR=Brest, BYVI=Vitebsk, BYHR=Grodno, BYMA=Mogilev, BYMI=Minsk, BYHM=City of Minsk).

Требования:
- В режиме **Цветы**, после выбора растения и нажатия "Найти цветы", под блоком результатов поиска показать карту Беларуси
...
- Данные для покраски берутся из результатов поиска (CitySearchResult[]), сортировка по floweringStart

Что уже есть:
- SVG-карта предоставлена пользователем, нужно сохранить в проект
- API `/api/search?mode=plants&slug=...` возвращает `{ mode: "plants", items: CitySearchResult[] }` с `floweringStart` по каждому городу
