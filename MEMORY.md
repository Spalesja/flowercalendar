...
## Следующий этап: 13 — Блок "Сейчас цветет"

План 13 этапа (из docs/dev-plan.md):
- По умолчанию показывать Минск
- При выборе другого города в режиме "Города" — обновлять блок
- Показывать до 3 карточек растений, цветущих сегодня
- Если меньше 3 — показывать сколько есть
- Если нет — аккуратное пустое состояние
- Карточки должны быть кликабельны (ссылка на страницу растения — возможно отложить до этапа 14)
